...
res=optimize_AX_XB(A_matrices, B_matrices, x_init)

res - T_cb


function X = optimize_AX_XB(A_matrices, B_matrices, x_init)
    % 将初始变换矩阵转换为李代数向量
    %x_init = decompose_X(X_init);
    
    % 定义优化问题的目标函数
    objective = @(x) objective_function(x, A_matrices, B_matrices);
    ...
    % 将优化结果转换为变换矩阵X
    X = compose_X(x_opt);
    final_error = objective_function(x_opt, A_matrices, B_matrices);
