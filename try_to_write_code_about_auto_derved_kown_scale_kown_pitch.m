...
%qx qz qy
%% test
AA=q2R(quaternion_multiply(qy,qz))'
BB=q2R(qy).'*q2R(qz).'
simplify(  AA-BB)
...
J = coef_J * mon_J_pure.' - 0.5 * lambda * (ac*ac + as*as - 1) - 0.5* mu * (gc*gc + gs*gs - 1);
...
function results = display_exp(expr, ac, as, gc, gs)
    % 获取所有的项
    terms = children(expr);

    % 初始化一个cell数组来存储结果
    results = cell(length(terms), 5);
    ...
    for i = 1:length(terms)
        ...
        % 找到ac, as, gc, gs的幂次
        [ac_coef, ac_power] = coeffs(term, ac);
        [as_coef, as_power] = coeffs(term, as);
        [gc_coef, gc_power] = coeffs(term, gc);
        [gs_coef, gs_power] = coeffs(term, gs);

        % 计算系数
        coef = subs(ac_coef, [as gc gs], [1 1 1]);

        % 将结果存入cell数组
        results{i, 1} = char(coef);
        results{i, 2} = ac_power;
        results{i, 3} = as_power;
        results{i, 4} = gc_power;
        results{i, 5} = gs_power;
    end
end