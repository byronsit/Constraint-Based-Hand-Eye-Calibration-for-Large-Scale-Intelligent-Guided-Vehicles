...
% 数据生成器是这么生成R_cprime_c的
% R_diff_pitch = eul2rotm([0, deg2rad(delta_p), 0], 'YXY');
% R_diff_yaw   = eul2rotm([0, 0, deg2rad(alpha)], 'ZYZ');
% R_diff_roll  = eul2rotm([deg2rad(gama), 0, 0], 'XYX');
R_diff_pitch = eul2rotm([0, deg2rad(delta_p), 0], 'ZYX');
R_diff_yaw   = eul2rotm([0, 0, deg2rad(alpha)], 'ZYZ');
R_diff_roll  = eul2rotm([deg2rad(gama), 0, 0], 'XYZ');
...
T_cprime_c = [R_cprime_c,t_c_cprimbe;0  0 0 1 ];         %这个数值似乎没错
...
%% 自动求导程序，也需要一样的功能。我需要大致验证这个东东


qy = [cosd(delta_p/2), 0, sind(delta_p/2), 0];
qz = [cosd(alpha/2), 0, 0, sind(alpha/2)];
qx = [cosd(gama/2),sind(gama/2),0,0];
q_b_R_cprime_c = (q2R(qy)')* (q2R(qz)')* (q2R(qx)'); %和quat2rotm对应，老吴写的q2R是逆似乎。可能遵循某些规则我不懂?
q_b_R_cprime_c - b_R_cprime_c


R_diff_yaw   = eul2rotm([0, 0, deg2rad(alpha)], 'ZYZ')
qz = [cosd(alpha/2), 0, 0, sind(alpha/2)]
quat2rotm(qz) - R_diff_yaw
...
% qy = [cosd(delta_p/2), 0, sind(delta_p/2), 0];
% quat2rotm(qy)
% 为啥我的R_diff_pitch和quat2rotm(qy)矩阵不一样？R_diff_pitch的表达里，只有y的旋转，其他都是0. 而且R_diff_pitch矩阵的1在左上角，但是quat2rotm(qy)的1在中间？
% 我哪里理解错了？不都是绕y轴旋转嘛？
